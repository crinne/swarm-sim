...
int main()
{
    ...
    UdpSocket socket;
    if (!check(socket.open(), "failed to open UDP socket")) {
        return 1;
    }
    int raw_fd = socket.fd();
    if (!check(raw_fd >= 0, "UDP socket fd was invalid")) {
        return 1;
    }

    UdpSocket moved(std::move(socket));
    if (!check(socket.fd() == -1, "moved-from UDP socket kept ownership")) {
        return 1;
    }
    if (!check(moved.fd() == raw_fd, "moved UDP socket lost fd")) {
        return 1;
    }

    moved.close();
    if (!check(::close(raw_fd) == -1,
               "closed UDP socket fd was still open after RAII close")) {
        return 1;
    }
