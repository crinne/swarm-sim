...
void test_http_status_and_cors()
{
    ...
    auto kill = client.Post("/kill", R"({"id":7})", "application/json");
    require(kill, "POST /kill did not return a response");
    require(kill->status == 200, "POST /kill did not return 200");
    require(kill->get_header_value("Access-Control-Allow-Origin")
                == "https://swarmgcs.dev",
            "POST /kill returned an unexpected allowed origin");
    require(server.killed_id() == 7,
            "POST /kill did not call the kill callback");
...
void test_gcs_tracks_battery_status()
{
    ...
    require(snap.drones[7].battery == 73.0f,
